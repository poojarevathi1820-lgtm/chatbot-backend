...
def format_top_selling(results_json: str) -> str:
    try:
        ...
        html_responses = ["<h3>Here are our top selling cases! 🔥</h3><div style='display:flex;flex-wrap:wrap;gap:16px;'>"]
...
def format_new_arrivals(results_json: str) -> str:
    try:
        ...
        results_to_display = results[:3]
        html_responses = [
            "<h3>Check out our latest arrivals! 🆕</h3><div style='display:flex;flex-wrap:wrap;gap:16px;'>"
        ]
        for case in results_to_display:
            case_id = case.get('id', '')
            name = case.get('name', 'N/A')
...
def fetch_order_history(user_id: str):
    try:
        response = supabase.table("orders").select(
            "items, total_cost, status, user_id"
...
def format_order_history(orders):
    ...
    html = "<b>Your Order History:</b><br><table border='1'><tr><th>Items</th><th>Total Cost</th><th>Status</th></tr>"
    for order in orders:
        items = order.get('items', 'N/A')
        total = order.get('total_cost', 'N/A')
        status = order.get('status', 'N/A')
        html += f"<tr><td>{items}</td><td>{total}</td><td>{status}</td></tr>"
    html += "</table>"
    return html
...
async def chat(chat_request: ChatRequest):
    ...
    return ChatResponse(reply="<p>I didn't understand your request.</p>")
